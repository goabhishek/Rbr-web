...
function DisplayPost({ post }) {
  const { content, mediaURL, email, createdAt } = post;
  ...
  useEffect(() => {
    if (users.length > 0) setUserDetails(users.filter((user) => user.email === email)[0]);
  }, [users, email]);
  ...
  const copyHandler = () => {
    navigator.clipboard.writeText(`https://bakingram.netlify.app/post/${post._id}`);
        ...
        <Flex {...postCardStyle} {...displayCardStyle}>
            ...
            {authUser.email === userDetails.email && (
              <Menu>
                <MenuButton pos='absolute' right='8' bg='transparent' color='gray.700' _hover={{ bg: 'transparent' }}>
                  <FontAwesomeIcon icon='ellipsis-h' size='2x' style={{ ...fontAwesomeIconStyle, padding: '5px' }} />
                </MenuButton>
                <MenuList minW='8rem'>
                  <MenuGroup>
          ...
          <Link to={`/post/${post._id}`}>
            ...
            {mediaURL && mediaURL.split('/')[4] === 'image' ? (
              <Image src={mediaURL} maxHeight='20rem' objectFit='fill' marginLeft='0' width='100%' />
            ) : (
              mediaURL && (
                <video controls>
                  <source src={mediaURL} />
          ...
          <HStack alignSelf='flex-start'>
            <Flex alignItems='center' flexDirection='col'>
              <IconButton
                ...
                icon={
                  likedByUser() ? (
                    <FontAwesomeIcon icon='heart' style={{ color: '#E53E3E' }} />
                  ) : (
            ...
            <IconButton
              variant='iconButton'
              icon={bookmarkedByUser() ? <FontAwesomeIcon icon='bookmark' /> : <FontAwesomeIcon icon={faBookmark} />}
            ...
            <IconButton variant='iconButton' icon={<FontAwesomeIcon icon='share-alt' />} onClick={copyHandler} />
          </HStack>
        </Flex>
      )}
    </>
