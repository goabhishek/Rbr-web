  ...
  return (
    <>
      <Flex
        ...
        pb='1'
      >
        <Link to={`/profile/${user.email}`} onClick={onClose}>
          <Avatar src={user.avatarUrl} alt='profile-image' size='sm' marginRight='2' name={user.name} />
        </Link>
        <Flex wrap='wrap'>
          <Link to={`/profile/${user.email}`} onClick={onClose}>
            <Flex width='100%' flexDirection='column' flexGrow='1'>
              <Text w={{ base: 'fit-content', md: '13ch' }}>{`${user.name} ${user.lastName}`}</Text>
