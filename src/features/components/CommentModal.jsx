...
function CommentModal({ postId, comment, editCommentContent }) {
  ...
  return (
    <>
      {!editCommentContent && (
        <IconButton variant='iconButton' onClick={onOpen} icon={<FontAwesomeIcon icon={faComment} />} />
      )}
      {editCommentContent && (
        <MenuItem _hover={{ bg: 'gray.300' }} bg='inherit' fontSize='md' onClick={onOpen}>
          Edit
        </MenuItem>
      )}
      <Modal isOpen={isOpen} onClose={onClose} isCentered size='md' m='2' motionPreset='slideInRight'>
        <ModalOverlay />
