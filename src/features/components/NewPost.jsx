...
function NewPost({ width, boxShadow, onClose, editPostContent }) {
  ...
  const [content, setContent] = useState(editPostContent ? editPostContent.content : '');
  ...
  const onPostClick = () => {
    if (!content && !mediaURL) toast.error('Please add content to post');
      ...
      else dispatch(addPost({ postData: { content, mediaURL, deleteToken }, authToken }));
      setContent('');
      setDeleteToken(null);
      setMediaURL('');
      setFilename('');
      editPostContent ? toast.success('Updated Successfully!') : toast.success('Posted successfully');
      onClose && onClose();
    }
  };
  return (
    <Flex {...postCardStyle} px='4' w={width} boxShadow={boxShadow}>
      <HStack alignItems='flex-start'>
        <Avatar src={authUser.avatarUrl} size='md' name={authUser.name} marginRight='3' mt='2' />
        <Textarea
      ...
      <Flex mx='2' justifyContent='space-between' height='10'>
        <HStack>
          <FormControl width='1rem'>
            <FormLabel position='absolute' left='0' bottom='0' cursor='pointer' marginBottom='0'>
              <FontAwesomeIcon icon='image' style={fontAwesomeIconStyle} />
            </FormLabel>
            <Input type='file' visibility='hidden' accept='image/*, video/*' onChange={onUploadClick} />
          </FormControl>
      ...
      {mediaURL && (
        <Tag size='md' borderRadius='full' variant='solid' maxW='fit-content' colorScheme='gray'>
          <TagLabel>{editPostContent?.mediaURL ? 'Media' : filename}</TagLabel>
          <TagCloseButton
            background='transparent'
            onClick={() => (deleteToken ? deleteMedia(deleteToken, setMediaURL, setDeleteToken) : setMediaURL(''))}
          />
        </Tag>
      )}
    </Flex>
