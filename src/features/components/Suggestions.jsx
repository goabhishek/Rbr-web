...
function Suggestions() {
  ...
  useEffect(() => {
    setSuggestions(
      users.filter(
        (currUser) =>
          !authUser.following.find((innerCurrUser) => innerCurrUser._id === currUser._id) &&
          currUser.email !== authUser.email
      )
    );
  }, [authUser, users]);

  return (
    <Flex {...suggestionContainerStyle} borderColor={useColorModeValue('gray.300', 'gray.700')}>
      {location.pathname === '/' && (
        <Flex justifyContent='space-around' mb='2' wrap='wrap' rowGap='10px'>
          <Button
            variant='outline'
            w='7rem'
            backgroundColor={postSorting === 'trending' ? ('purple.100', 'gray.800') : 'none'}
          ...
          <Button
            variant='outline'
            w='7rem'
            backgroundColor={postSorting === 'latest' ? ('purple.100', 'gray.800') : 'none'}
      ...
      {suggestions
        .slice(0, 5)
        .map((user) => user.email !== authUser.email && <UserCard user={user} key={user._id} followButton={true} />)}
    </Flex>
  );
}
