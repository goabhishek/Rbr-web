    ...
    <>
      ...
      >
        <Link to={`/profile/${user.username}`} onClick={onClose}>
          <Avatar src={user.avatarUrl} alt='profile-image' size='sm' marginRight='2' name={user.firstName} />
        </Link>
        <Flex wrap='wrap'>
          <Link to={`/profile/${user.username}`} onClick={onClose}>
            <Flex width='100%' flexDirection='column' flexGrow='1'>
              <Text w={{ base: 'fit-content', md: '13ch' }}>{`${user.firstName} ${user.lastName}`}</Text>
